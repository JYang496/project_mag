	...
	if timeout and dest != null:
		...
		if player.global_position.distance_to(dest.global_position) < 5:
			...
			if PhaseManager.current_state() == PhaseManager.PREPARE:
				if not is_connected("enable_enemy_spawner",Callable(GlobalVariables.enemy_spawner,"start_timer")):
					connect("enable_enemy_spawner",Callable(GlobalVariables.enemy_spawner,"start_timer"))
