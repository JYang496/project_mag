...
@export var elite_progress_weight: float = 0.5
...
func set_task_parameters(params: Dictionary) -> void:
	...
	if params.has("clear_allow_elite"):
		allow_elite = bool(params["clear_allow_elite"])
	if params.has("clear_elite_weight"):
		elite_progress_weight = float(params["clear_elite_weight"])
...
func _spawn_quest_enemies() -> void:
	...
	var elite_candidates: Array[SpawnInfo] = []
	var normal_candidates: Array[SpawnInfo] = []
	for info in spawn_infos:
		if info == null:
			continue
		if _is_elite_spawn(info):
			elite_candidates.append(info)
		else:
			normal_candidates.append(info)

	var elite_spawned := false
	if allow_elite and not elite_candidates.is_empty():
		var elite_info: SpawnInfo = elite_candidates.pick_random() as SpawnInfo
		var elite_enemy := _spawn_enemy_from_info(elite_info)
		if elite_enemy:
			_register_quest_enemy(elite_enemy, maxf(elite_progress_weight, 0.0))
			elite_spawned = true

	var normal_count: int = max(quest_enemy_count, 0)
	for _i in range(normal_count):
		var pick_list: Array[SpawnInfo] = normal_candidates if not normal_candidates.is_empty() else spawn_infos
		var info: SpawnInfo = pick_list.pick_random() as SpawnInfo
	...
	if total_weight <= 0.0 and elite_spawned:
		total_weight = maxf(elite_progress_weight, 0.0)
