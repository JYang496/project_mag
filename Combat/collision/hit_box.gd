...
func apply_attack(area) -> void:
	...
	var damage_deferred := false
	if hitbox_owner and hitbox_owner.has_method("try_defer_hit_damage"):
		damage_deferred = bool(hitbox_owner.call(
			"try_defer_hit_damage",
			target,
