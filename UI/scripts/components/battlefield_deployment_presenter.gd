...
func _build_overlay(root: Control) -> void:
	overlay = Control.new()
	overlay.name = "BattlefieldDeploymentOverlay"
	...
	scrim.color = Color(0.005, 0.025, 0.045, 0.0)
	scrim.mouse_filter = Control.MOUSE_FILTER_IGNORE
	overlay.add_child(scrim)

	scan_line = ColorRect.new()
	scan_line.set_anchors_preset(Control.PRESET_TOP_WIDE)
	scan_line.offset_left = 28.0
	scan_line.offset_top = 92.0
	scan_line.offset_right = -28.0
	scan_line.offset_bottom = 94.0
	scan_line.color = Color(0.35, 0.86, 1.0, 0.82)
	...
	status_label.add_theme_color_override("font_color", Color(0.64, 0.91, 1.0, 1.0))
	column.add_child(status_label)
	var progress_track := ColorRect.new()
	progress_track.custom_minimum_size = Vector2(0.0, 3.0)
	progress_track.color = Color(0.07, 0.19, 0.25, 0.90)
	...
	progress_fill.color = Color(0.30, 0.77, 0.92, 1.0)
...
func _build_status_style() -> StyleBoxFlat:
	...
	style.bg_color = Color(0.015, 0.055, 0.085, 0.92)
	style.border_color = Color(0.25, 0.70, 0.84, 0.82)
	style.set_border_width_all(1)
	style.set_corner_radius_all(5)
	style.shadow_color = Color(0.0, 0.02, 0.04, 0.62)
	style.shadow_size = 6
	return style
