...
const ENTRY_BUTTON_SCENE := preload("res://UI/components/PrimaryMenuEntry/PrimaryMenuEntry.tscn")
...
func _rebuild_buttons(entries: Array) -> void:
	...
	for entry in entries:
		var button := ENTRY_BUTTON_SCENE.instantiate() as Button
		button.name = str(entry.get("node_name", "EntryButton%d" % (_buttons.size() + 1)))
		button.call("set_data", str(entry.get("text", "")))
