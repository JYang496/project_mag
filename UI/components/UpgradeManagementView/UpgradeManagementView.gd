...
const UPGRADE_ITEM_SCENE := preload("res://UI/components/UpgradeItemCard/UpgradeItemCard.tscn")
const MODULE_ROW_SCENE := preload("res://UI/components/UpgradeModuleRow/UpgradeModuleRow.tscn")
const EMPTY_STATE_SCENE := preload("res://UI/components/EmptyStateLabel/EmptyStateLabel.tscn")
const FUSION_LEVEL_STATUS_SCENE := preload("res://UI/components/FusionLevelStatus/FusionLevelStatus.tscn")
const FUSION_STAGE_SCENE := preload("res://UI/components/FusionStageIndicator/FusionStageIndicator.tscn")
const FUSION_CONDITION_SCENE := preload("res://UI/components/FusionConditionTile/FusionConditionTile.tscn")
...
func refresh_template() -> void:
	...
	if items.is_empty():
		...
		empty.call("set_data", LocalizationManager.tr_key("ui.upgrade.empty", "No upgradeable items."))
...
func ensure_item_list_layout() -> void:
	assert(upgrade_item_list is VBoxContainer)

func create_module_row() -> HBoxContainer:
	var row := MODULE_ROW_SCENE.instantiate() as HBoxContainer
...
func add_item_row(item_data: Dictionary, parent_container: Container = null) -> void:
	...
	var maximum := int(item_data.get("max_level", 0))
...
func _add_fusion_status_row(level: int, required_level: int) -> void:
	var row := FUSION_LEVEL_STATUS_SCENE.instantiate() as Control
...
func _add_fusion_step_header(step: int, text: String) -> void:
	...
	label.call("set_data", "%d  %s" % [step, text], Color(0.58, 0.86, 1.0), 15)
...
func _add_fusion_branch_button(branch: WeaponBranchDefinition, enhanced: bool, parent: Container) -> void:
	...
	button.call("set_data", str(branch.branch_id), "%s%s\n%s" % [LocalizationManager.get_branch_display_name(branch), suffix, _format_tags(branch.get_normalized_fusion_required_tags())], selected_fusion_branch_id == str(branch.branch_id))
	button.connect("selected", _select_fusion_branch)
	if owner_ui:
...
func _add_fusion_core_row(stack: Dictionary, recipe: Array[StringName], required_count: int) -> void:
	...
	var row := FUSION_CORE_ROW_SCENE.instantiate() as Control
	upgrade_detail_body.add_child(row)
	var info_text := "%s  x%d\n%s: %s  ·  %s: %s  ·  %s: %d" % [
	...
	row.call("set_data", {"key": key, "text": info_text, "can_remove": chosen > 0, "can_add": not contribution.is_empty() and chosen < int(stack.get("count", 0)) and selected_core_keys.size() < required_count, "tooltip": LocalizationManager.tr_key("ui.fusion.unrelated", "This core does not contribute a required Tag.") if contribution.is_empty() else ""})
	row.connect("amount_changed", _change_core_selection)
...
func _add_fusion_label(text: String, color: Color = Color(0.82, 0.88, 0.9), size: int = 13) -> void:
	var label := DETAIL_TEXT_SCENE.instantiate() as Label
	upgrade_detail_body.add_child(label)
	label.call("set_data", text, color, size)
