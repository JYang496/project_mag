...
const WEAPON_CARD_SCENE := preload("res://UI/components/ModuleEquipWeaponCard/ModuleEquipWeaponCard.tscn")
...
func _build_weapon_row(parent: Container, weapon: Weapon) -> void:
	...
	for index in range(weapon.module_slot_capacity):
		var installed := equipped_modules[index] as Module if index < equipped_modules.size() else null
		socket_data.append({"icon": _get_node_texture(installed, "%Sprite") if installed != null else null, "tooltip": _build_module_tooltip(installed) if installed != null else "", "occupied": installed != null})
	card.call("set_sockets", socket_data, TOKENS.COLOR_ACCENT_SYSTEM, TOKENS.COLOR_BORDER)
	card.call("start_pulse", TOKENS.MOTION_SLOW)
	card.connect("equip_pressed", _on_slot_selected.bind(weapon, replacement))
