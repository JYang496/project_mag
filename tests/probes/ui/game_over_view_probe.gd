...
func _run() -> void:
	...
	_expect(_view.layer > 80, "结算层必须高于玩家受伤反馈 CanvasLayer。")
	...
	_expect(_view.get_node("GameOverRoot").mouse_filter == Control.MOUSE_FILTER_STOP,
		"结算根节点必须阻止输入穿透。")
	_expect(_view.title_label.text == "挑战失败", "中文结算标题未正确刷新。")
	_expect(_view.subtitle_label.text == "本局战绩", "中文统计副标题未正确刷新。")
	_expect(_view.total_damage_value.text == "1,234,567", "大数值应使用千位分隔符。")
	_expect(_view.enemy_kills_value.text == "85", "敌人击杀数未正确呈现。")
	_expect(_view.new_game_button.text == "返回主菜单", "按钮文案应匹配实际行为。")
	_expect(_view.new_game_button.has_focus(), "结算显示后应默认聚焦主操作按钮。")
	_expect(_view.game_over_panel.custom_minimum_size.x <= 560.0,
		"结算面板不应保留过宽的空白区域。")

	print("FAIL game-over view probe" if _failed else "PASS game-over view probe")
