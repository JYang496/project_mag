...
func _test_accessible_heat_hud() -> void:
	...
	var layout: Dictionary = meter.get_heat_layout_state()
	var track: Rect2 = layout.get("track", Rect2())
	var cold_end_cap: Rect2 = layout.get("cold_end_cap", Rect2())
	var hot_end_cap: Rect2 = layout.get("hot_end_cap", Rect2())
	var cell_size: Vector2 = layout.get("cell_size", Vector2.ZERO)
	var cell_y := float(layout.get("cell_y", 0.0))
	var hot_cell_x: Array = layout.get("hot_cell_x", [])
	_expect(layout.get("layout") == &"bipolar_rail", "Heat HUD must use the selected bipolar rail layout")
	_expect(bool(layout.get("uses_generated_texture", false)), "Heat HUD must render the selected generated frame texture")
	_expect(str(layout.get("texture_path", "")) == "res://UI/themes/modern/heat_rail_frame_generated_v4.png", "Heat HUD must bind the plaque-free frame asset")
	_expect(int(layout.get("slot_count", 0)) == 8, "Heat HUD must use eight wide readable energy chambers")
	_expect(cell_size == Vector2(11.0, 11.0) and is_equal_approx(cell_y, 38.0), "Heat fills must match the V4 texture's 11x11 inner chambers")
	_expect(hot_cell_x == [116.0, 131.0, 146.0, 161.0], "hot fill origins must align with the V4 texture chambers")
	_expect(not bool(layout.get("status_text_visible", true)), "Heat HUD must not render a lower status container or text")
	_expect(layout.get("full_feedback") == &"internal_vent_pulse", "full Heat feedback must use the end-cap vents instead of a rectangular warning frame")
	_expect(meter.custom_minimum_size == Vector2(218.0, 78.0), "bipolar Heat HUD must keep its compact runtime footprint")
	_expect(cold_end_cap.end.x < track.position.x, "cold identity must be integrated into a separate left end cap")
	_expect(hot_end_cap.position.x > track.end.x, "hot identity must be integrated into a separate right end cap")
	_expect(is_equal_approx(float(layout.get("neutral_x", 0.0)), track.get_center().x), "Heat rail neutral gate must stay centered")
	...
	var appearance_meter = COMBAT_RESOURCE_METER.new()
	appearance_meter.visible = false
	add_child(appearance_meter)
	await get_tree().process_frame
	appearance_meter.set_resource(&"heat", 0.85, &"high_heat", "+70", "hot")
	appearance_meter.show_animated()
	_expect(is_equal_approx(appearance_meter.get_display_ratio(), 0.85), "Heat HUD must open at the authoritative temperature instead of sweeping from maximum cold")
	_expect(appearance_meter.scale.x < 1.0, "Heat HUD show animation must begin horizontally collapsed")
	var hot_high: Dictionary = appearance_meter.get_heat_visual_state()
	appearance_meter.set_resource(&"heat", 0.72, &"hot", "+44", "hot")
	appearance_meter.show_animated()
	var hot_medium: Dictionary = appearance_meter.get_heat_visual_state()
	appearance_meter.set_resource(&"heat", 0.60, &"hot", "+20", "warm")
	appearance_meter.show_animated()
	var hot_low: Dictionary = appearance_meter.get_heat_visual_state()
	_expect(hot_low.get("color") != hot_medium.get("color") and hot_medium.get("color") != hot_high.get("color"), "Heat severity must progress through distinct yellow, orange, and red colors")
	appearance_meter.set_resource(&"heat", 0.40, &"cold", "-20", "cool")
	appearance_meter.show_animated()
	var cold_low: Dictionary = appearance_meter.get_heat_visual_state()
	appearance_meter.set_resource(&"heat", 0.28, &"cold", "-44", "cold")
	appearance_meter.show_animated()
	var cold_medium: Dictionary = appearance_meter.get_heat_visual_state()
	appearance_meter.set_resource(&"heat", 0.10, &"deep_cold", "-80", "deep cold")
	appearance_meter.show_animated()
	var cold_high: Dictionary = appearance_meter.get_heat_visual_state()
	_expect(cold_low.get("color") != cold_medium.get("color") and cold_medium.get("color") != cold_high.get("color"), "Cold severity must progress through three distinct ice-blue colors")
	_expect(not bool(hot_high.get("status_text_visible", true)), "Heat severity must be expressed by color without lower status text")
	await get_tree().create_timer(0.22).timeout
	_expect(appearance_meter.scale.is_equal_approx(Vector2.ONE), "Heat HUD show animation must finish fully expanded")
	appearance_meter.hide_animated()
	_expect(appearance_meter.is_hiding_animated(), "Heat HUD hide animation must enter the collapsing state")
	await get_tree().create_timer(0.16).timeout
	_expect(not appearance_meter.visible, "Heat HUD hide animation must finish hidden")
	appearance_meter.queue_free()
	meter.queue_free()
