...
func _run() -> void:
	...
	await get_tree().process_frame
	await get_tree().process_frame
	ui.call("_init_module_action_dialogs")
	await get_tree().process_frame
	...
	if not ui.request_temporary_module_sell_confirmation(sell_module):
		...
		return
	await get_tree().process_frame
	ui.module_action_dialog.get_ok_button().pressed.emit()
	await get_tree().process_frame
	await get_tree().process_frame
	...
	_restore(_ui)
	print("PASS temporary module settlement confirmation")
	await TEST_TEARDOWN.finish(self, 0, _restore_runtime_state)
	_ui = null
...
func _fail(message: String) -> void:
	...
	push_error(message)
	print("FAIL: ", message)
	await TEST_TEARDOWN.finish(self, 1, _restore_runtime_state)
	_ui = null
