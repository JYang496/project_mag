...
const PROFILES := {
	...
	&"charged_blaster_phase_echo": {"name_key": "ui.weapon.skill.charged_blaster_phase_echo.name", "description_key": "ui.weapon.skill.charged_blaster_phase_echo.description", "fallback_name": "Phase Echo", "fallback_description": "The next charged attack repeats in the same direction after a short delay. The echo preserves its branch pattern, deals 65% damage, and lasts 60% as long.", "duration": 0.0},
	&"spear_phalanx": {"name_key": "ui.weapon.skill.spear_phalanx.name", "description_key": "ui.weapon.skill.spear_phalanx.description", "fallback_name": "Spear Phalanx", "fallback_description": "Fire 8 returning spears in a 40-degree spread toward the target. Surviving enemies are knocked back.", "duration": 0.0},
	...
	&"cannon_siege_trajectory": {"name_key": "ui.weapon.skill.cannon_siege_trajectory.name", "description_key": "ui.weapon.skill.cannon_siege_trajectory.description", "fallback_name": "Walking Barrage", "fallback_description": "Call four mortar strikes that walk across the target area from near to far, dealing 50%, 60%, 70%, and 90% weapon damage.", "duration": 0.0},
