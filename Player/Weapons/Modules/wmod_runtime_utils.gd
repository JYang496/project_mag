...
static func get_nearby_enemies(tree: SceneTree, origin: Vector2, radius: float) -> Array[Node2D]:
	...
	if tree == null:
		return output
	for enemy_ref in EnemyRegistry.get_enemies_in_radius(origin, radius):
...
static func get_enemy_candidates(tree: SceneTree) -> Array[Node2D]:
	...
	if tree == null:
		return output
	for enemy_ref in EnemyRegistry.get_enemies():
		var enemy := enemy_ref as Node2D
		if enemy != null and is_instance_valid(enemy):
			output.append(enemy)
	return output
...
static func get_enemies_in_rect(tree: SceneTree, world_rect: Rect2) -> Array[Node2D]:
	...
	if tree == null:
		return output
	for enemy_ref in EnemyRegistry.get_enemies_in_rect(world_rect):
		var enemy := enemy_ref as Node2D
		if enemy != null and is_instance_valid(enemy):
			output.append(enemy)
	return output
