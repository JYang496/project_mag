extends Module
# Use on HEAT weapons to apply a fire DOT payload on each hit.
...
func apply_on_hit(source_weapon: Weapon, target: Node) -> void:
	...
	if target.has_method("apply_status_payload"):
		target.apply_status_payload(&"dot", {
			"damage": final_damage,
			"tick": ticks,
			"damage_type": &"fire",
		})
	...
	if target.has_method("apply_status_effect"):
		...
		var effect := DotStatusEffect.new().setup_dot_effect(ticks, final_damage, Attack.TYPE_FIRE)
