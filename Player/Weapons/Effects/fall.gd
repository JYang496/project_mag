...
func projectile_effect_ready() -> void:
	...
	projectile.call_deferred("add_sibling",p0)
	projectile.call_deferred("add_sibling",p1)
	projectile.call_deferred("add_sibling",p2)
	_helpers_added = true
...
func _physics_process(delta: float) -> void:
	...
	if projectile.global_position.distance_to(p2.global_position) > 5:
		t += delta * (speed / 200.0)
		projectile.global_position = _quadratic_bezier(t)
	else:
		arrived = true
		_cleanup_helpers()
