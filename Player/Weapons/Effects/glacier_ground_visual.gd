...
func _draw() -> void:
	...
	var life: float = area.get("_elapsed_sec") / area.get("duration_sec")
	var enhanced: bool = area.get("cold_snap_active")
	var crumble := clampf((life - 0.8) / 0.2, 0, 1)
	...
	for column in range(columns):
		for row in range(3):
			...
			for corner in [Vector2(inset,1),Vector2(tile_width-2,0),Vector2(tile_width-1,width/3-3),Vector2(2,width/3-1),Vector2(0,4)]:
				var point: Vector2 = Vector2(x,y)+corner+drift
			...
			if not projectable:
				continue
			draw_colored_polygon(points,Color(0.28+variant*0.03,0.64,0.83,0.32*(1-crumble)))
			var crack := PackedVector2Array()
			for offset in [Vector2(3,3),Vector2(tile_width*0.55, width/6),Vector2(tile_width-3,width/3-3)]:
				crack.append(_project(Vector2(x,y)+offset+drift,view))
			draw_polyline(crack,Color(0.7,0.92,1,0.55*(1-crumble)),1)
			if enhanced and variant % 2 == 0:
